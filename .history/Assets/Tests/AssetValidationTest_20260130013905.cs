...
public class AssetValidationTest
{
    ...
    public void Factory_Assets_Have_Valid_Prefabs()
    {
        ...
        foreach(string guid in guids)
        {
            ...
            // CHECK IF CHARACTER PREFAB GAME OBJECT IS PRESENT IN SCRIPTABLE OBJECT
            Assert.IsFalse(string.IsNullOrEmpty(characterData.characterName), $"Error: CharacterData at {path} has no name. Please check the name field, It will lead to ");
            ...
            Assert.GreaterOrEqual(characterData.health, 100, $"Error: CharacterData at {path} having low health than minimum character health standard");
        }
    }
}
